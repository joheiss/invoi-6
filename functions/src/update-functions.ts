...
export async function handleReceiverCreation(
...
export async function handleReceiverDeletion(
  snap: DocumentSnapshot,
  context: EventContext
): Promise<any> {
  // reset number range
  ...
  return deleteDocumentLinksForOwner(
    `${snap.data().objectType}/${context.params.id}`
  );
}
...
export async function handleContractDeletion(
  snap: DocumentSnapshot,
  context: EventContext
): Promise<any> {
  try {
...
export async function handleInvoiceDeletion(
  snap: DocumentSnapshot,
  context: EventContext
): Promise<any> {
  try {
    ...
    // delete assigned document links
    await deleteDocumentLinksForOwner(
      `${snap.data().objectType}/${context.params.id}`
    );
    // update revenue reporting
...
export async function handleUserProfileCreation(
  snap: DocumentSnapshot,
  context: EventContext
): Promise<any> {
...
export async function handleUserProfileDeletion(
  snap: DocumentSnapshot,
  context: EventContext
): Promise<any> {
  try {
    ...
    return admin
      .auth()
      .setCustomUserClaims(uid, { isAdmin: false, isSales: false });
  } catch (err) {
    console.log(err);
  }
}
...
export async function handleDocumentLinkDeletion(
  snap: DocumentSnapshot,
  context: EventContext
): Promise<any> {
  try {
...
function isInvoiceUpdateRelevantForRevenue(
  newInvoiceData,
  oldInvoiceData
): boolean {
...
async function resetNumberRange(
  rangeId: string,
  context: EventContext
): Promise<any> {
...
async function updateNumberRange(
  rangeId: string,
  context: EventContext
): Promise<any> {
  const id = context.params.id;
...
async function updateAuth(uid: string, userProfile: any): Promise<any> {
  try {
    await admin.auth().updateUser(uid, {
      ...
      photoURL:
        userProfile.imageUrl && userProfile.imageUrl.length
    ...
    return admin
      .auth()
      .setCustomUserClaims(uid, { isAdmin: isAdmin, isSales: isSales });
  } catch (err) {
    throw new Error(err);
  }
}
async function updateRevenueReporting(
  newInvoiceData: any,
  oldInvoiceData: any
): Promise<any> {
  ...
  if (values.new.year) {
    ...
    if (!revenues) {
      ...
      revenues = {
        ...
        months: {
          [values.new.month]: { [values.new.receiverId]: values.new.revenue },
        },
      };
    } else if (!revenues.months[values.new.month]) {
      // create new entry for month
      revenues.months[values.new.month] = {
        [values.new.receiverId]: values.new.revenue,
      };
    } else if (!revenues.months[values.new.month][values.new.receiverId]) {
      // create new entry for receiver
      revenues.months[values.new.month][values.new.receiverId] =
        values.new.revenue;
    } else {
  ...
  if (values.old.year) {
    ...
    if (
      revenues &&
      revenues.months[values.old.month] &&
      revenues.months[values.old.month][values.old.receiverId]
    ) {
      // correct old entry
