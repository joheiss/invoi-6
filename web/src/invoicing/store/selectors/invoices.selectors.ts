...
import {InvoiceFactory, InvoiceStatus} from 'jovisco-domain';
...
export const selectAllInvoicesAsObjArray = createSelector(
  selectAllInvoices,
  invoices => InvoiceFactory.fromDataArray(invoices)
);
...
export const selectCurrentInvoiceAsObj = createSelector(
  ...
  invoice => invoice && InvoiceFactory.fromData(invoice)
);
...
export const selectOpenInvoicesAsObjArray = createSelector(
  selectOpenInvoices,
  invoices => invoices.map(invoice => InvoiceFactory.fromData(invoice))
);
